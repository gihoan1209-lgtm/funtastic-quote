        ...
        <div className="quote-paper">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0', marginBottom: '1.25rem', border: '1px solid var(--border)' }}>
            <div style={{ borderRight: '1px solid var(--border)' }}>
              ...
              <div style={{ padding: '8px 12px', fontSize: '13px', borderBottom: '1px solid var(--border)' }}>
            ...
            <div style={{ display: 'flex' }}>
              <div style={{ background: '#1a1a1a', color: '#fff', writingMode: 'vertical-lr', textOrientation: 'upright', fontSize: '13px', fontWeight: 600, letterSpacing: '0.2em', padding: '12px 6px', display: 'flex', alignItems: 'center', justifyContent: 'center', minWidth: '28px' }}>공급자</div>
              <div style={{ flex: 1, borderLeft: '1px solid var(--border)' }}>
                <div className="info-row" style={{ borderBottom: '1px solid var(--border)' }}>
                  ...
                  <div className="info-val"><input className="editable-inline" value={supplier.reg} onChange={e => updateSupplier('reg', e.target.value)} style={{ fontSize: '12px', width: '100%', textAlign: 'center' }} /></div>
                </div>
                <div className="info-row" style={{ borderBottom: '1px solid var(--border)' }}>
                  <span className="info-label" style={{ minWidth: '40px' }}>상&nbsp;&nbsp;호</span>
                  <div className="info-val" style={{ flex: 1 }}><input className="editable-inline" value={supplier.name} onChange={e => updateSupplier('name', e.target.value)} style={{ fontSize: '12px', width: '100%' }} /></div>
                  <span className="info-label" style={{ minWidth: '40px', borderLeft: '1px solid var(--border)' }}>성&nbsp;&nbsp;명</span>
                  <div className="info-val" style={{ flex: 1 }}><input className="editable-inline" value={supplier.rep} onChange={e => updateSupplier('rep', e.target.value)} style={{ fontSize: '12px', width: '100%' }} /></div>
                </div>
                <div className="info-row" style={{ borderBottom: '1px solid var(--border)' }}>
                  <span className="info-label" style={{ minWidth: '64px' }}>사업장 주소</span>
                  <div className="info-val"><input className="editable-inline" value={supplier.addr} onChange={e => updateSupplier('addr', e.target.value)} style={{ fontSize: '12px', width: '100%' }} /></div>
                </div>
                <div className="info-row" style={{ borderBottom: '1px solid var(--border)' }}>
                  <span className="info-label" style={{ minWidth: '40px' }}>업&nbsp;&nbsp;태</span>
                  <div className="info-val" style={{ flex: 1 }}><input className="editable-inline" value={supplier.biz} onChange={e => updateSupplier('biz', e.target.value)} style={{ fontSize: '12px', width: '100%' }} /></div>
                  <span className="info-label" style={{ minWidth: '40px', borderLeft: '1px solid var(--border)' }}>종&nbsp;&nbsp;목</span>
                  <div className="info-val" style={{ flex: 1 }}><input className="editable-inline" value={supplier.type} onChange={e => updateSupplier('type', e.target.value)} style={{ fontSize: '12px', width: '100%' }} /></div>
                </div>
                <div className="info-row">
                  <span className="info-label" style={{ minWidth: '64px' }}>전화번호</span>
                  <div className="info-val"><input className="editable-inline" value={supplier.tel} onChange={e => updateSupplier('tel', e.target.value)} style={{ fontSize: '12px', width: '100%' }} /></div>
                </div>
              </div>
            </div>
          </div>
              ...
              {items.map((item, idx) => {
                ...
                return (
                  <tr key={item.id}>
                    ...
                    <td>
                      <textarea className="editable" value={item.name} onChange={e => updateItem(item.id, 'name', e.target.value)} placeholder="품명" rows={1} style={{ resize: 'none', overflow: 'hidden', lineHeight: '1.4', minHeight: '26px', width: '100%' }}
                    ...
                    <td>
                      <input className="editable" value={item.note} onChange={e => updateItem(item.id, 'note', e.target.value)} />
                    </td>
